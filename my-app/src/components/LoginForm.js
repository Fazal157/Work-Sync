...
export default function LoginForm({ onSwitch, onSuccess }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      // Check localStorage for registeredUsers
      const users = JSON.parse(localStorage.getItem('registeredUsers') || '[]');
      const user  = users.find(u => u.email === form.email);

      if (!user) {
        setAlert({ type: 'error', msg: 'No account found with this email. Please sign up first.' });
        setLoading(false);
        return;
      }
      if (!user.verified) {
        setAlert({ type: 'error', msg: 'Please verify your email before logging in. Check your inbox.' });
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="aform">
      <div className="aform__head">
        <h1 className="aform__title">Welcome back 👋</h1>
      ...
      <button className="aform__submit" onClick={handleLogin} disabled={loading}>
        {loading ? <><span className="aform__spinner" /> Signing in...</> : 'Sign In'}
      </button>

      <div className="aform__divider">or continue with</div>

      <div className="aform__socials">
        <button className="aform__social-btn">
